...
from settings import AVAILABLE_PREFIXES, CONFIG, DEFAULT_ASSIGNEE, DEFAULT_TESTER, DEFAULT_PREFIX, DEFAULT_TIMEOUT_MS, NAVIGATION_TIMEOUT_MS
...
class JiraDestPage(BasePage):
    ...
    def set_default_people(self):
        ...
        for label, selectors, person in (
            ("Ответственный", ("#responsible-field",), DEFAULT_ASSIGNEE),
            ("Тестировщик", (DJ["tester"],), DEFAULT_TESTER),
        ):
            if self.set_person_if_present(label, selectors, person):
                login_logger.info("system=destination_jira event=person_field_set field=%s name=%s", label, person)
