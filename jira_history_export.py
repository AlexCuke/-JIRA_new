...
def enrich_sheet_values(page, base_url, issue_key, values):
    """Добавляет срок, статус и версии независимо от полей маппинга формы."""
    results, failures = fetch_issue_statuses(page, base_url, [issue_key])
    if failures:
        raise RuntimeError(f"Не удалось прочитать данные {issue_key} для таблицы: {failures[issue_key]}")
    data = results[issue_key]
    values.update({"Срок_Исполнения": data["duedate"], "Версия": data["fix_versions"], "Статус": data["status"]})
    return values
